...
def require_tank_access(min_level: str):
    """FastAPI dependency factory — like require_owned_tank, but also accepts a tank
    shared with the current user at `min_level` or above, or one belonging to a group
    the current user is a member of (group access is uniform full access, so it
    satisfies any min_level)."""
    def dependency(tank_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> Tank:
        ...
        if tank and tank.owner_id != user.id:
            in_group = tank.group_id is not None and tank.group_id in user_group_ids(db, user.id)
            if not in_group:
