...
def list_daily_tasks(tank_id: str, db: Session = Depends(get_db), _tank: Tank = Depends(require_owned_tank)):
...
def create_daily_task(tank_id: str, body: DailyTaskCreate, db: Session = Depends(get_db), _tank: Tank = Depends(require_owned_tank)):
...
def update_daily_task(tank_id: str, task_id: str, body: DailyTaskUpdate, db: Session = Depends(get_db), _tank: Tank = Depends(require_owned_tank)):
...
def delete_daily_task(tank_id: str, task_id: str, db: Session = Depends(get_db), _tank: Tank = Depends(require_owned_tank)):
    task = db.query(DailyTask).filter_by(id=task_id, tank_id=tank_id).first()
    if not task:
        raise HTTPException(404, "Task not found")
    db.delete(task)
