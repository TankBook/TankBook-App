...
def list_health_cases(tank_id: str, db: Session = Depends(get_db), _tank: Tank = Depends(require_tank_view)):
    cases = (
        db.query(HealthCase)
...
def add_health_case(tank_id: str, body: HealthCaseCreate, db: Session = Depends(get_db), _tank: Tank = Depends(require_tank_edit)):
    fish = None
...
def update_health_case(tank_id: str, case_id: str, body: HealthCaseUpdate, db: Session = Depends(get_db), _tank: Tank = Depends(require_tank_edit)):
...
def delete_health_case(tank_id: str, case_id: str, db: Session = Depends(get_db), _tank: Tank = Depends(require_tank_edit)):
    case = db.query(HealthCase).filter_by(id=case_id, tank_id=tank_id).first()
    if not case:
        raise HTTPException(404, "Case not found")
    db.delete(case)
