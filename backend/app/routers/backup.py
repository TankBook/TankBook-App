...
def import_backup(payload: dict, db: Session = Depends(get_db)):
    ...
    for t in payload.get("tanks", []):
        tank = Tank(
            ...
            co2_source=t.get("co2_source"), co2_method=t.get("co2_method"),
            has_heater=t.get("has_heater", False), heater_watts=t.get("heater_watts"),
            has_lighting=t.get("has_lighting", False),
            light_intensity=t.get("light_intensity"), light_watts=t.get("light_watts"),
            light_technology=t.get("light_technology"),
