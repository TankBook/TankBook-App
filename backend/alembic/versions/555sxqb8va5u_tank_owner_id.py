...
authenticated user. This adds a required owner_id FK to users, backfilling
existing tanks to the instance's original account (the earliest-created user)
so nothing pre-existing loses access.
...
def upgrade() -> None:
    ...
    """)
    op.alter_column('tanks', 'owner_id', nullable=False)
